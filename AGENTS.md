...
- По умолчанию запускай только проверки, которые не создают сборочные
  артефакты: например, `cd client && npm run lint`, точечные тесты и
  `node --check` для серверных файлов.
- Не запускай `npm run build`, `vite build`, production-сборку или другие
  команды, создающие `dist`, `build` и аналогичные каталоги, без прямого
  запроса пользователя.
- Не используй сборку как стандартную проверку после изменений. Если без неё
  нельзя подтвердить результат, сначала сообщи об этом пользователю и получи
  явное разрешение.
- Не удаляй и не изменяй существующие сборочные артефакты, если это не является
  частью прямого запроса пользователя.
